...
void GW_GameEngine_VTech_Monkey::do_update()
{
    if (GetMode()==MODE_TIME1 || GetMode()==MODE_TIME2 || GetMode()==MODE_ALARM || GetMode()==MODE_DATE)
        clock_update();
}

void GW_GameEngine_VTech_Monkey::do_turnon()
{
    SetMode(MODE_IDLE);
}

void GW_GameEngine_VTech_Monkey::do_turnoff()
{
    SetMode(MODE_OFF);
}
...
void GW_GameEngine_VTech_Monkey::game_update()
{
    //char_update(char_position_, false);
}
...
void GW_GameEngine_VTech_Monkey::score_update()
{
    int p=score_;
...
void GW_GameEngine_VTech_Monkey::miss_update()
{
    for (int i=1; i<=10; i++)
    {
...
bool GW_GameEngine_VTech_Monkey::do_setmode(int mode)
{
    switch (mode)
    {
    case MODE_OFF:
...
void GW_GameEngine_VTech_Monkey::clock_update(int mode)
{
    if (mode==-1) mode=GetMode();
...
void GW_GameEngine_VTech_Monkey::setnumber(int n, int ps1, int ps2, bool leadzero)
{
